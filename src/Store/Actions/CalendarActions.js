...
export const winnersRoundFourAction = (round) => async (dispatch) => {
  try {
    dispatch({
      type: WINNERS_ROUND_FOUR_REQUEST,
    });
    const options = {
      method: 'GET',
      url: `https://f1-three.onrender.com/api/winners/2023/${round}`,
    ...
    dispatch({ type: WINNERS_ROUND_FOUR_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: WINNERS_ROUND_FOUR_FAILURE,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
...
export const winnersRoundFiveAction = (round) => async (dispatch) => {
  try {
    dispatch({
      type: WINNERS_ROUND_FIVE_REQUEST,
    });
    const options = {
      method: 'GET',
      url: `https://f1-three.onrender.com/api/winners/2023/${round}`,
    ...
    const { data } = await axios.request(options);
    dispatch({ type: WINNERS_ROUND_FIVE_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: WINNERS_ROUND_FIVE_FAILURE,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
